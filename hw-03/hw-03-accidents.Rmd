...
ggplot(data = accidents, aes(x = time, fill = severity)) +
  ...
  theme_minimal() +
  labs(
    title = "Accident distributions",
    subtitle = "Severity of weekends vs weekdays",
    x = "Time of day",
    y = "Density",
    fill = "Severity"
  )
...
- When do accidents tend to occur during weekdays vs. weekends? There are considerably more fatal and serious accidents during weekdays than weekends.
- How does the severity of accidents vary by time of day? They wave throughout the day, but peak slightlty as it gets past 4pm.
- Are there any notable differences between weekdays and weekends? I don't see any fatal crash reports on the weekend.
...
ggplot(data = accidents, aes(x = speed_limit, y = vehicles, color = severity)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "lm") +
  theme_minimal() +
  labs(
    title = "Relationship between speed limit, the vehicles, and accident severity",
    x = "Speed limit (mph)",
...
**What question does your visualization answer?** My visualization shows the connection between number of vehicles in the accident, along with the speed limit, to compare if they lean more fatal, serious, or slight.


**What variables were used:** I used 3 variables.

**Interpretation (3-4 sentences):** Oddly enough my graph is showing that fatal accidents are mainly occurring on low speed limit areas. That would lead me to assume that this is due to people speeding on roads they shouldn't be. I also noticed that many of these fatal accidents include more than two cars in the collision.
